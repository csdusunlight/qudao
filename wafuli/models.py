...
class Project(models.Model):
    ...
    category = models.CharField(u"项目属性", max_length=20, choices=Project_CATE, default='self')
...
class InvestLog(models.Model):
    ...
    delta_amount = models.DecimalField(u'补差金额（已计入结算金额）', max_digits=6, decimal_places=2, default=0)
...
class MAdvert_PC(Base):
    ...
    def clean(self):
        if self.pic and self.pic.size > 300000:
                raise ValidationError({'pic': u'图片大小不能超过300k'})
